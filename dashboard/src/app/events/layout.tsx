...
export default function EventsLayout({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex h-screen bg-[#0C0F0E] overflow-hidden text-slate-300 font-sans">
            {/* Sidebar - Blue theme for Standard Events */}
            <aside className="w-[260px] bg-[#121615] border-r border-[#1C2220] flex flex-col justify-between hidden lg:flex z-20 py-6">
                <div>
                    <div className="flex items-center gap-3 px-6 mb-8">
                        <div className="w-8 h-8 rounded-full bg-[#73A9F0] flex items-center justify-center shadow-[0_0_15px_rgba(115,169,240,0.2)]">
                            <Activity size={18} className="text-[#0A0C0B]" />
                        </div>
                        <span className="font-bold text-lg text-white">EventCore</span>
                    </div>

                    <div className="px-4">
                        <p className="px-4 text-[10px] font-bold text-slate-500 tracking-wider mb-2">MENU</p>
                        <nav className="space-y-1">
                            <Link href="/events" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-500/10 text-blue-400 font-medium transition-all group">
                                <LayoutDashboard size={18} className="group-hover:text-blue-300 transition-colors" />
                                <span>Dashboard</span>
                            </Link>
                            <Link href="/events/list" className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:text-white hover:bg-[#1A201E] font-medium transition-all group">
                                <Map size={18} className="group-hover:text-white transition-colors" />
                                <span>Eventos</span>
                            </Link>
                            <Link href="/events/calendar" className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:text-white hover:bg-[#1A201E] font-medium transition-all group">
                ...
                <div className="px-6 mt-4">
                    <div className="bg-gradient-to-br from-[#1A2220] to-[#0D1211] border border-[#252E2B] rounded-2xl p-5 relative overflow-hidden group">
                        ...
                        <p className="text-[10px] text-slate-400 mb-4">Gerencie eventos pelo celular</p>
                        <button className="w-full py-2 bg-blue-500/10 text-blue-400 text-xs font-bold rounded-lg hover:bg-blue-500 hover:text-[#0C0F0E] transition-all border border-blue-500/20">
                            Visualizar Download
                        </button>
                    </div>
                </div>
            ...
            <main className="flex-1 flex flex-col h-screen relative overflow-hidden bg-[#0C0F0E]">
                ...
                <header className="h-[88px] flex flex-col justify-center px-8 z-10 shrink-0">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center bg-[#121615] px-4 py-2.5 rounded-2xl w-[320px] border border-[#1C2220] focus-within:border-blue-500/50 focus-within:shadow-[0_0_15px_rgba(115,169,240,0.05)] transition-all">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3 pl-2 bg-[#121615] border border-[#1C2220] rounded-full pr-1.5 py-1.5">
                                <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-700">
                                    <img src="https://i.pravatar.cc/150?u=eventcore" alt="Avatar" className="w-full h-full object-cover" />
                                </div>
                                <div className="hidden sm:block pr-3">
                                    <p className="text-xs font-bold text-white leading-tight">Organizador</p>
                                    <p className="text-[10px] text-slate-400">Evento Padrão</p>
                                </div>
                            </div>
                        </div>
                    </div>

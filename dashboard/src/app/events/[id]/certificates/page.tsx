    ...
    return (
        <div className="space-y-6 fade-in max-w-[1400px] mx-auto pb-6 h-full flex flex-col">
            ...
            <header className="flex justify-between items-end pb-4 border-b border-[#1C2220] shrink-0">
                ...
                <div className="w-12 h-12 rounded-2xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20 shadow-[0_0_20px_rgba(59,130,246,0.15)]">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 pt-4 flex-1">
                ...
                <div className="space-y-6 flex flex-col">
                    <div className="bg-[#121615] p-8 rounded-3xl border border-[#1C2220] shadow-lg relative overflow-hidden flex-1 group">
                        <div className="absolute -top-10 -left-10 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl group-hover:bg-blue-500/20 transition-all"></div>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                                    <div className="bg-[#0A0C0B] p-3 rounded-xl border border-[#1C2220]">
                                        <span className="block font-mono text-xs text-blue-300 mb-1">&#123;&#123;nome_participante&#125;&#125;</span>
                                        <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Nome Completo do Participante</span>
                                    </div>
                                    <div className="bg-[#0A0C0B] p-3 rounded-xl border border-[#1C2220]">
                                        <span className="block font-mono text-xs text-blue-300 mb-1">&#123;&#123;evento_nome&#125;&#125;</span>
                                        <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Título do Evento</span>
                                    </div>
                                    <div className="bg-[#0A0C0B] p-3 rounded-xl border border-[#1C2220]">
                                        <span className="block font-mono text-xs text-blue-300 mb-1">&#123;&#123;data_realizado&#125;&#125;</span>
                                        <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Data do Evento</span>
                                    </div>
                                    <div className="bg-[#0A0C0B] p-3 rounded-xl border border-[#1C2220]">
                                        <span className="block font-mono text-xs text-blue-300 mb-1">&#123;&#123;codigo_verificacao&#125;&#125;</span>
                                        <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Hash de Verificação</span>
                                    </div>
                                </div>

                                <div className="flex bg-[#0A0C0B] border-2 border-dashed border-[#1C2220] rounded-2xl overflow-hidden hover:border-blue-500/50 transition-colors p-2 focus-within:border-blue-400">
                                    <input
                                        type="file"
                                        accept=".docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                                        onChange={handleFileChange}
                                        className="w-full px-4 py-4 text-sm text-slate-400 file:mr-4 file:py-2 file:px-6 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-500/10 file:text-blue-400 hover:file:bg-blue-500/20 cursor-pointer"
                                    />
                                </div>
                                {certificateFile && (
                ...
                <div className="space-y-6 flex flex-col">
                    <div className="bg-gradient-to-br from-[#121615] to-[#0A0C0B] p-8 rounded-3xl border border-[#1C2220] shadow-lg flex-1 flex flex-col justify-center items-center text-center">
                        <div className="w-20 h-20 bg-[#1A201E] rounded-full border border-[#1C2220] flex items-center justify-center mb-6 shadow-xl">
                            <Zap size={32} className={certificateFile ? "text-blue-400" : "text-slate-600"} />
                        </div>
                        <h3 className="text-2xl font-bold text-white mb-2">Processamento em Lote</h3>
                        <p className="text-slate-400 max-w-sm mb-8">
                            {certificateFile
                                ? "O modelo está pronto. Ao iniciar, o sistema lerá os dados de todos os participantes confirmados, preencherá as variáveis no Word e converterá tudo em um único arquivo PDF/ZIP."
                        ...
                        <button
                            ...
                            className={`px-8 py-4 font-bold rounded-xl text-lg flex items-center gap-3 shadow-lg transition-all ${certificateFile
                                ? generating
                                    ...
                                    : 'bg-blue-500 text-white hover:bg-blue-400 hover:scale-105 shadow-[0_0_20px_rgba(59,130,246,0.3)]'

    ...
    return (
        <div className="space-y-6 fade-in max-w-[1400px] mx-auto pb-6 h-full flex flex-col">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-[#1C2220]">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Calendário de Eventos</h1>
                    <p className="text-slate-400 mt-1 text-sm font-medium">Visualize todos os seus eventos programados.</p>
                </div>
                        ...
                        return (
                            <div key={day} className={`p-2 border-r border-b border-[#1C2220]/50 relative min-h-[100px] sm:min-h-[120px] hover:bg-[#1A201E]/50 transition-colors ${isToday ? 'bg-[#4F46E5]/10' : ''}`}>
                                ...
                                <div className="mt-2 space-y-1">
                                    {dayEvents.map(evt => (
                                        <div key={evt.id} className="text-[9px] sm:text-[10px] bg-blue-500/10 border border-blue-500/20 text-blue-400 px-1 sm:px-2 py-1 sm:py-1.5 rounded-md truncate cursor-pointer hover:bg-blue-500/20 transition-colors font-semibold" title={evt.name}>
                                            {evt.name}
                                        </div>
                                    ))}
                                </div>

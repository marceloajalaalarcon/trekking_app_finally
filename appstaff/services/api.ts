...
export async function processStaffCheckin(barcode: string) {
    const res = await authFetch('/staff-checkin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
        ...
        throw new Error(error.message || 'Erro ao processar ingresso');
    }
    return res.json();
}

export async function fetchMyStaffRoles() {
    const res = await authFetch('/users/me/staff-roles');
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        console.log('[Staff Roles] HTTP', res.status, err);
        throw new Error(err.message || `Erro ao carregar permissões (${res.status})`);
    }
    return res.json();
}

export async function updateTrekkingSettings(eventId: string, data: any) {
    const res = await authFetch(`/trekkings/${eventId}`, {
        method: 'PUT',
        body: JSON.stringify(data),
    });
    if (!res.ok) {
